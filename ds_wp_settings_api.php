...
//name of a JSON file.
class DustySun_WP_Settings_API {
	...
	//Create the object
	public function __construct($ds_wp_json_file = null, $ds_wp_register_settings = false) {

		// Read the JSON file with the settings that makes everything work
		$this->read_json_file($ds_wp_json_file);

		// Set plugin defaults
		$this->set_plugin_options();

		// Register the settings if true - if we're building an options page
		if ($ds_wp_register_settings) add_action( 'admin_init', array( $this, 'build_settings' ) );
	...
	public function set_plugin_options($update_db = false) {
		...
		//only set up the plugin settings]
		...
		$this->current_settings = $this->current_settings();
		...
		if(is_admin()) {
			...
			//Get the views for s//set our plugin settings var now that we have any stored views
			...
			//the info key
			if($update_db) {
				$ds_wp_settings_api_plugin_options_key = $this->plugin_settings['plugin_domain'] . '_plugin_settings';

				$ds_wp_settings_api_plugin_settings_except_views = $this->plugin_settings;
				$ds_wp_settings_api_plugin_settings_except_views['info'] = '';

				update_option($ds_wp_settings_api_plugin_options_key, $ds_wp_settings_api_plugin_settings_except_views);
			} // end update_db
	...
	public function register_ds_wp_settings_api_admin_styles_scripts($hook) {
		...
		if($hook == $this->plugin_settings['page_hook']) {
			...
			//Google fonts
			wp_enqueue_style('ds-wp-google-fonts', 'https://fonts.googleapis.com/css?family=Open+Sans:400,600,700');
			...
			if($tabs == "true") {
				...
				<h2 class="nav-tab-wrapper">
						...
						foreach($ds_wp_settings_api_tab_array as $ds_wp_settings_api_section_key => $ds_wp_settings_api_section) {
							...
							?>
							<a href="?page=urgency-coupons-mailing-lists&tab=<?php echo $ds_wp_settings_api_section_slug; ?>" class="nav-tab <?php if($active_tab == $ds_wp_settings_api_section_slug){echo 'nav-tab-active';} ?> "><?php _e($tab_label, 'sandbox'); ?></a>
	...
	public function current_settings($update_db = false) {
	...
	public function build_settings() {
		...
		foreach ($this->ds_wp_settings_api_fields as $ds_wp_settings_api_setting_id => $ds_wp_settings_api_field_setting) {
			...
			foreach ($ds_wp_settings_api_field_setting['fields'] as $ds_wp_settings_api_field_setting) {
				...
				//Skip any items not marked user user_editable
				if(isset($ds_wp_settings_api_field_setting['user_editable']) && $ds_wp_settings_api_field_setting['user_editable'] == 'false') {	continue; }
	...
	public function ds_wp_settings_api_sanitize( $raw_input_data_fields )
	{
		...
		$option_key = $raw_input_data_fields['ds_wp_settings_api_option_key'];

		//get the saniziation type
		$sanitization_fields = $this->ds_wp_settings_api_fields[$option_key]['fields'];
